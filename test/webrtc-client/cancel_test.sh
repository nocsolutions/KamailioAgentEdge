...
#   AST_HOST=<asterisk-ip> EXT=<ext> cancel_test.sh <password>
PW="${1:?need password}"
SP="$(cd "$(dirname "$0")" && pwd)"
...
NOANSWER=1 node "$SP/wsclient/agent.js" "wss://216.66.20.147:4443/ws" ${EXT:-21382} "$PW" avatar.tech >/tmp/ct_agent.log 2>&1 &
AP=$!
sleep 6
